...
class Game extends Component {
  ...
  handleChange = (e) => {
    ...
    if (e.target.value !== "" && !alreadyTried.includes(e.target.value)) {
      // fulls in an array with the previous tried letter and so avoid already tried letter
      alreadyTriedCloned.push(e.target.value);
    }

    if (wordToFind.includes(e.target.value.toUpperCase())) {
      // fulls in an array with matched letters with the word to find
      goodletterCloned.push(e.target.value.toUpperCase());
      document.querySelector("input").style.boxShadow = "0px 0px 4px green";
    } else {
      // changes the background so to display each step of an hangman
      document.querySelector("input").style.boxShadow = "0px 0px 4px red";

      document.querySelector("#hangman").style.background =
        "url(" + hangTab[1] + ") no-repeat";
      document.querySelector("#hangman").style.backgroundPosition =
        "-250px -55px";
    ...
    this.setState({
      curentLetter: curentLetterCloned, // state's uploading
  ...
  winnerDance = () => {
    // displaying winning message if the word is found
    ...
    if ([...this.state.words[0]].length === letterCount.length) {
      // proposes a new word to guess
        ...
        <>
          <div className="d-flex m-5 p-5 border border-success rounded">
            <div className="mx-auto">
              ...
              <p className="m-5">
                Your score : {this.state.alreadyTried.length}{" "}
              </p>
              <button
  ...
  looserDance = () => {
    // displaying loosing message if the hangman is completed

    const errorCountCloned = this.state.errorCount;

    if (errorCountCloned.length === 8) {
      document.querySelector("#hangman").style.background =
        "url(" + this.state.hangTab[0] + ") no-repeat";
      return (
        <>
  ...
  changeWord = () => {
    // takes off the first word of the array of words to guess so that a new word is behind the expression 'this.state.words[0]]'
      ...
      <>
        ...
        <div className="row">
          <div style={divStyle} className="col-6">
            <div className="row">
              <label style={labelStyle} className="col-3 mt-5 pl-5 pt-1">
                Try a letter :{" "}
              </label>
              <input
                type="text"
